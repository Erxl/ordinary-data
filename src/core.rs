...
macro_rules! declare {
    ($ty_ptr:ident,$ty:ident,$ty_data:ident) => {
        ...
        {
            #[inline]
            pub unsafe fn data(&self) -> &$ty_data {
                &self.0.as_ref().data
            }
            #[inline]
            pub unsafe fn data_mut(&self) -> &mut $ty_data {
            ...
            #[inline]
            #[allow(dead_code)]
            unsafe fn get_mut(&self) -> &mut args!($ty) {
                &mut *self.as_ptr()
            }
            #[inline]
            #[allow(dead_code)]
            unsafe fn get(&self) -> &args!($ty) {
                self.0.as_ref()
            }
            #[inline]
            #[allow(dead_code)]
    ...
    Container<ConceptData, RelationData, RelationTypeData>
{
    ...
    pub unsafe fn delete_concept(&mut self, concept: args!(ConceptPtr)) {
        ...
        //todo 可优化;
        c.relation_type_to_dst_relation
            .values()
            .collect::<Vec<_>>()
            .into_iter()
            .for_each(|x| self.delete_relation(*x));
        c.src_to_relation
            .values()
            .collect::<Vec<_>>()
            .into_iter()
    ...
    pub unsafe fn create_relation_with_data<
        'a,
        DstConceptsIter: Clone + Iterator<Item = &'a args!(ConceptPtr)>,
    >(
        &'a mut self,
        relation_type: args!(RelationTypePtr),
        src: args!(ConceptPtr),
            ...
            .entry(relation_type_key)
        {
            Entry::Vacant(entry) => {
                //创建关系
                let relation_ref =
                    RelationPtr::new_from_ref(self.relations.entry(key).or_insert(Relation {
                        key,
                        data,
                ...
                let relation_type_dst_to_relation_ref =
                    &mut relation_type.get_mut().dst_to_relations;
    ...
    pub fn contains_concept(&mut self, concept: args!(ConceptPtr)) -> bool {
        self.concepts.values().any(
            #[inline]
            |x| concept.as_ptr() == &*x as *const Concept<_, _, _> as _,
        )
    }
    #[inline]
    ...
    pub fn contains_relation(&mut self, relation: args!(RelationPtr)) -> bool {
        self.relations.values().any(
            #[inline]
            |x| relation.as_ptr() == &*x as *const Relation<_, _, _> as _,
        )
    }
    #[inline]
    ...
    pub fn contains_relation_type(&mut self, relation_type: args!(RelationTypePtr)) -> bool {
        self.relation_types.values().any(
            #[inline]
            |x| relation_type.as_ptr() == &*x as *const RelationType<_, _, _> as _,
        )
    }
    #[inline]
    ...
    ConceptPtr<ConceptData, RelationData, RelationTypeData>
{
    ...
    pub unsafe fn incoming(
        &self,
        relation_type: args!(RelationTypePtr),
    ) -> Option<&BTreeMap<u64, args!(RelationPtr)>> {
